...
import {Tabbar,TabbarItem,GoodsAction,GoodsActionBigBtn,GoodsActionMiniBtn,NavBar,Uploader,Area,DatetimePicker,Search,Tab,Tabs,Button,Icon,Swipe,Dialog,SwipeItem,Rate,Stepper,SwitchCell,NoticeBar,RadioGroup,Row,PasswordInput, NumberKeyboard, Col,Card,Popup,AddressList,AddressEdit,Collapse,CollapseItem, Radio ,Checkbox, CheckboxGroup,PullRefresh,List,Toast,Field,Cell,CellGroup,Loading } from 'vant'
...
Vue.prototype.$url = 'http://jinzhongapi.test.zhikewl.com'//测试地址
//console.log(Vue.prototype.$url)
Vue.use(VueCookies)
Vue.use(Toast)
Vue.use(GoodsAction).use(GoodsActionBigBtn).use(GoodsActionMiniBtn).use(Uploader).use(Area).use(DatetimePicker).use(Search).use(Tab).use(Tabs).use(Button).use(Icon).use(Swipe).use(SwipeItem).use(Rate).use(Stepper).use(SwitchCell)
.use(RadioGroup).use(Row).use(PasswordInput).use(NumberKeyboard).use(Col).use(Card).use(Popup).use(AddressList).use(AddressEdit).use(Collapse).use(CollapseItem).use(Radio).use(Checkbox).use(CheckboxGroup).use(PullRefresh).use(List)
